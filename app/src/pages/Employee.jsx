...
import EmployeeTopNav from '../components/Employee/Layout/EmployeeTopNav';
import EmployeeSideNav from '../components/Employee/Layout/EmployeeSideNav';
import EmployeeBottomNav from '../components/Employee/Layout/EmployeeBottomNav';
...
const Employee = () => {
  ...
  return isLoading ? (
    ...
    <div style={{ backgroundColor: crmDetails?.theme.background, color: crmDetails?.theme.text.primary }} className='flex flex-col min-h-screen w-full'>
      {crmDetails?.layout === "Top Navigation" && <EmployeeTopNav name={crmDetails?.name} id={id} theme={crmDetails?.theme.navbar} />}
      {crmDetails?.layout === "Sidebar Focused" && <EmployeeSideNav name={crmDetails?.name} id={id} open={open} setOpen={setOpen} theme={crmDetails?.theme.navbar} />}
      <div className={`
          flex-1 
          ...
          `}><Outlet /></div>
      {crmDetails?.layout === "Bottom Navigation" && <EmployeeBottomNav id={id} theme={crmDetails?.theme.navbar} />}
    </div>
  )
}
